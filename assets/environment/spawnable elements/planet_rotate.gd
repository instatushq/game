...
func _physics_process(_delta: float) -> void:
	...
	var camera_y = int(round(camera.global_position.y))
	var motion_mirroring_y = int(round(motion_mirroring.y))
	var absolute_normalized_distance = abs(camera_y % motion_mirroring_y)
	if name.contains("Big"):
		print(camera_y % motion_mirroring_y)
		
	if absolute_normalized_distance <= 2:
